...
4. **配置对话摘要额外提示词**（Operit 平台，PreCompact 确定性实现）
   - 运行 `python3 install.py --print-summary-prompt` 获取提示词全文
   - 或直接读取 `references/summary-prompt.md` 复制
   - 用户粘贴到平台"对话摘要额外提示词"配置入口
   - 效果：每次上下文压缩时平台自动附加 → 摘要强制六段式 + 悬挂线程不丢（L1 平台强制，不依赖模型自觉）
5. **执行验证清单**
   - `query_memory("[铁律] 强制执行清单")` 是否命中？（引用不带版本号，标题固定）
   - 说"检查马具"能否列出 10 事件注册表？
   - 记忆数量 = manifest 条数（以 manifest 为准，当前 9 条）？
   - 摘要提示词已配置？（等待一次压缩后检查摘要是否含 ①~⑦ 结构）
6. **完成提示**：告知安装结果 + 可选删除 install_manifest.json
